...
<body>
	...
	<c:if test="${isAdmin}">
		<div class="container">
			<div class="row">
				<h1>Gerenciar Perfil Prestadores Banidos</h1>
			</div>
		</div>
		<br />
		<br />
					...
					<tbody>
						<c:forEach items="${modelLogins}" var="ml">
							<tr>
								...
								<td><a href="<%=request.getContextPath()%>/ServletGerenciarSistema?acao=carregarPerfilPrestBanido&id_user=<%=session.getAttribute("id")%>&id=${ml.id}"><button type="button"
											class="btn btn-primary">Gerenciar</button></a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
		...
		<nav aria-label="Page navigation example" >
			...
			<ul class="pagination">
				...
				for (int p = 0; p < totalPagina; p++) {
					String url = request.getContextPath() + "/ServletGerenciarSistema?acao=carregarPerfilPrestsBanido&id_user="
